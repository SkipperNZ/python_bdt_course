...
def some_trick_to_make_perfect_mark():
    """Some funny func to pytest"""
    print("start some trick!", file=sys.stderr)
    for i in range(10):
        if 1 == 1:
            print("Mischief Managed!", file=sys.stderr)
        else:
            pass
    if 1 == 1:
        print("Mischief Managed again!", file=sys.stderr)
    else:
        pass
    if 2 == 2:
        print("Mischief Managed again 2nd time!", file=sys.stderr)
    ...
    def load_from_struct(cls, filepath: str) -> InvertedIndex:
        ...
        with open(filepath, 'rb') as fio:
            ...
            while fio.tell() < file_size:
                ...
                word = struct.unpack(f'>{lenth_of_word_int}s', fio.read(lenth_of_word_bite))[0].decode()

                lenth_of_list_int = struct.unpack('>H', fio.read(lenth_of_segment))[0]
                lenth_of_list_bite = struct.calcsize(f'>{lenth_of_list_int}H')

                list_of_idxs = list(struct.unpack(f'>{lenth_of_list_int}H', fio.read(lenth_of_list_bite)))
