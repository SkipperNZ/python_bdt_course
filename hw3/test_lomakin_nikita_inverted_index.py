...
from task_lomakin_nikita_inverted_index import *
...
def test_constructor():
    ...
    assert idx._idx_storage == {'first': [1, 2, 3], 'second': [
        4, 5, 6]}, "constructor doesn't work"
...
def test_build_inverted_index():
    ...
    assert sorted(correct._idx_storage) == sorted(
        idx._idx_storage), "Wrong  build inverted index"
...
def test_query():
    correct_query = [3]
    idx = InvertedIndex({'first': [1, 2, 3], 'second': [
                        4, 5, 6], 'third': [7, 3, 9]})
...
def test_can_encoded_file_read(monkeypatch):
    monkeypatch.setattr('sys.stdin', io.TextIOWrapper(io.BytesIO(b"test")))
    encoded_type = EncodedFileType("r", encoding="utf-8")
    text_io = encoded_type('-')
    assert io.TextIOWrapper == text_io.__class__
